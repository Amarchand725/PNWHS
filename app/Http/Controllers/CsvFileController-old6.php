    ...
    public function store(Request $request)
    {
        ...
        if(!empty($csv_raw_file)){
            ...
            foreach($csv_raw_data2 as $record){
                ...
                //         $total_amount = $member_details->hasPromotedMember->hasPromotedRank->hasPolicy->hasPaymentPolicy->registration_payment+$member_details->hasPromotedMember->hasPromotedRank->hasPolicy->hasPaymentPolicy->insurance_payment;
                //         // $total_paid_amount = (int)$record->monthly_amount+(int)$record->reg_fee+(int)$record->insurance;
                //         $total_paid_amount = $members_paid[$member_details->p_no];
                ...
                //                 'reg_insu_fee' => $total_paid_amount,
                ...
                //             if($members_paid[$member_details->p_no] < abs($member_payment->amount)){
                //                 $amount = $members_paid[$member_details->p_no]+$member_payment->amount;
                ...
                //                     'reg_insu_fee' =>  $members_paid[$member_details->p_no],
                //                     'submitted_amount' => $members_paid[$member_details->p_no]+$member_payment->submitted_amount,
                //                     'current_paid' => $members_paid[$member_details->p_no],
                //                     'sub_monthly_install' => $amount,
                //                     'amount' => $amount,
                //                 ]);
                //             }else if($members_paid[$member_details->p_no] == abs($member_payment->amount)){
                //                 $amount = $members_paid[$member_details->p_no]+$member_payment->amount;
                ...
                //                     'payment_type' => 'sheet',
                //                     'total_amount' => $member_payment->amount,
                //                     'reg_insu_fee' => $member_payment->amount,
                //                     'submitted_amount' => $members_paid[$member_details->p_no]+$member_payment->submitted_amount,
                //                     'current_paid' => $members_paid[$member_details->p_no],
                //                     'sub_monthly_install' => $amount,
                //                     'amount' => $amount,
                ...
                //             //After promoted payment
                //             $m_active_from = strtotime($member_details->d_o_p);
                //             $current = strtotime(now());
                ...
                //             $total_active_months = (($current_year - $active_year) * 12) + ($current_month - $active_month);
                //             $total_active_months_amount = $member_details->hasPromotedMember->hasPromotedRank->hasPolicy->hasPaymentPolicy->monthly_instalment*$total_active_months;
                //             $total_paid_installs = Payment::where('p_no', $request->p_no)->where('is_active', 1)->sum('sub_monthly_install');
                //             $last_amount2 = $total_paid_installs-$total_active_months_amount;
                            
                //             $amount1 = $last_amount+$last_amount2;
                            
                //             $amount = $total_paid_amount+$amount1;
                ...
                //                 'total_amount' => $last_amount,
                //                 'sub_monthly_install' => $total_paid_amount,
                //                 'submitted_amount' => $total_paid_amount+$total_paid_amount,
                //                 'current_paid' => $total_paid_amount,
                //                 'amount' => $amount,
                //                 'is_active' => 1,
                //             ]);
    ...
    public function parseFile(CsvImportRequest $request)
    {
        ...
        if($request->submit == 'monthly-sheet'){
            ...
            if (count($data) > 0) {
                ...
                foreach($data->toArray() as $k => $record){
                    $csv_raw_data = CsvRawData::create([
                        'raw_file_id' => $csv_raw_file->id,
                        's_no' => (int)$record['sno'],
                        'p_no_for_hony' => $record['p_no_of_hony'],
                        'cat' => $record['cat'],
                        'g_r' => (int)$record['g_r'],
                        'dd_ch' => $record['dd_ch'],
            ...
            if (count($data) > 0) {
                $csv_data = array_slice($data->toArray(), 1,1);
                foreach($csv_data as $key=>$csv){
                    $month_records = array_slice($csv_data[$key], 9);
                }
