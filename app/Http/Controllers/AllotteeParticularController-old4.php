...
class AllotteeParticularController extends Controller
{
    ...
    public function index()
    {
        ...
        if(isset($_GET['submit'])){
            ...
            return view('allotteeParticular.index', array('formModel' =>$formModel, 'models' => $models,'permissioninsert' => $permissioninsert , 'permissionview' => $permissionview ,'permissionupdate' => $permissionupdate , 'permissiondelete' => $permissiondelete, 'ranks' => $ranks ));
        }
        if (\Request::ajax()) {
     ...
     */
        ...
        public function getLastRegFileNo($id)
        {
            ...
            $last_reg_file_number = AllotteeParticular::orderby('id', 'DESC')->whereIn('rank_rate', $ranks)->first(['reg_file_no']);

            $category = HouseCategory::where('id', $rank->category)->first();
            $reg_file_no = '';
            if(empty($last_reg_file_number)){
                $reg_file_no = $category->start_range;
            }else{
                if($category->name == 'B'){
                    if($last_reg_file_number->reg_file_no < $category->end_range){
                        $reg_file_no = $last_reg_file_number->reg_file_no+1;
                    }else if($last_reg_file_number->reg_file_no < $category->start_range){
                        $reg_file_no = $category->start_range;
                    }else if($last_reg_file_number->reg_file_no < $category->end_range){
                        $reg_file_no = $last_reg_file_number->reg_file_no+1;
                    }else{
                        $reg_file_no = 'Limitation over';
                    }
                }if($category->name == 'D'){
                    $reg_file_no = $last_reg_file_number->reg_file_no+1;
                }else{
    ...
    public function update($id, Request $request)
    {
        ...
        if(!empty($promoted_rec)){
            ...
            $record = AllotteeParticular::where('p_no', $promoted_rec->old_p_no)->first();
    ...
    public function assigndata(){
        ...
        $members = AllotteeParticular::with('hasRank', 'hasUser', 'hasMemberStatus')->orderby('total_service', 'DESC')->where('plotassigned', 0)->get();
