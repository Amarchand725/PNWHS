...
class PromotedMemberController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $record = AllotteeParticular::where('p_no', $request->old_p_no)->orwhere('p_no', $request->new_p_no)->first();
        $tot_service = '';
        if($request->soldier=='civilian'){
    ...
    public function update($id, Request $request)
    {
        //
        $model = PromotedMember::findOrFail($id);
        $model->file_registration_no=null;
        $model->save();
        ...
        $promoted_record = PromotedMember::findorFail($id);
        $record = AllotteeParticular::where('id', $promoted_record->member_id)->first();
        $tot_service = '';
        if($request->soldier=='civilian'){
            $start_date = date_create($record->d_o_b);
            $end_date = date_create($request->d_o_s);
        ...
        if($id){
            ...
            $pay_policy = PaymentPolicyData::with('hasPaymentPolicy')->orderby('id', 'DESC')->where('rank_id', $model->promoted_rank_id)->first();

            $last = payableRegInsurance::orderby('id', 'DESC')->where('member_id', $model->member_id)->first();
            if(!empty($last)){
