...
class AllotteeParticularController extends Controller
{
    ...
    public function index()
    {
        ...
        if(isset($_GET['submit'])){
            ...
            $ranks = Rank::where('status', 1)->pluck('name', 'id');
            $models = AllotteeParticular::orderby('id', 'DESC')->with('hasPayment')->where('seen', 1)->paginate(10);
     ...
     */
        ...
        {
            $last_reg_file_number = AllotteeParticular::orderby('id', 'DESC')->where('rank_rate', $id)->first(['reg_file_no']);
            return $last_reg_file_number;
        }
    ...
    public function store(Request $request)
    {
        ...
        if(!empty($request->nextkins)){
            foreach($request->nextkins as $key=>$nextkin){
                if($nextkin['name']=='' || $nextkin['relation']=='' || $nextkin['cnic']=='' || $nextkin['mobilenumber'] == '' || $nextkin['country_code']=='' || $nextkin['share'] || $nextkin['kinaddress'] == ''){
        ...
        if(empty($ifnotexist)){
            ...
            if(!empty($request->transactionform2)){
                foreach ($request->transactionform2 as $key => $value) {
                    if($value['nextofkinfilefront'] != null){
                    ...
                    if($value['nextofkinfileback'] != null){
