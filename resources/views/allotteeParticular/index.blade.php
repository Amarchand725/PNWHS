							...
							<tr>
								...
								<td>
									@if(!empty($model->hasPromotedMember))
										{{ $model->hasPromotedMember->file_registration_no??'N/A' }}
									@else
								...
								<td>
									...
									@else
										{{ $model->hasRank->name??'N/A' }}
									@endif
								</td>
								<td>
									@if(!empty($model->hasPromotedMember))
										{{ $model->hasPromotedMember->soldier??'N/A' }}
									@else
										{{ $model->soldier??'N/A' }}
									@endif
								</td>
								<td>
									@if(!empty($model->hasPromotedMember))
										{{ $model->hasPromotedMember->hasPromotedRank->hasHouseCategory->name??'N/A' }}
									@else
										{{ $model->hasRank->hasHouseCategory->name??'N/A' }}
									@endif
								</td>
								...
								<td>
									@if($permissionview == 'true')
										...
										<a href="{{ route('AllotteeParticular.show', $model->id) }}" data-toggle="tooltip"  title="Show Application" class="btn btn-primary btn-sm"> <span class="glyphicon glyphicon-eye-open"></span></a>
									@endif
					...
					<tbody class='ajax_content'>
						...
						@foreach($records as $model)
							<tr>
								...
								<td>{{ $model->membership_date }}.</td>
								<td>{{ $model->honu_p_no??'N/A' }}</td>
								<td>{{ $model->name??'N/A' }}</td>
								<td>
									@if(!empty($model->hasPromotedMember))
										{{ $model->hasPromotedMember->hasPromotedRank->name??'N/A' }}
								...
								<td>
									@if(!empty($model->hasPromotedMember))
										{{ $model->hasPromotedMember->rank_rate??'N/A' }}
									@else
										{{ $model->branch??'N/A' }}
									@endif
								</td>
								...
								<td>
									{{ date('d, M-Y', strtotime($model->d_o_b))??'N/A' }}
								</td>
								<td>
									{{ date('d, M-Y', strtotime($model->d_o_e))??'N/A' }}
								</td>
								<td>
									{{ date('d, M-Y', strtotime($model->d_o_c))??'N/A' }}
								</td>
								<td>
									{{ date('d, M-Y', strtotime($model->d_o_sod))??'N/A' }}
								</td>
								<td>
									{{ date('d, M-Y', strtotime($model->d_o_sos))??'N/A' }}
								</td>
								<td>
									{{ date('d, M-Y', strtotime($model->d_o_s))??'N/A' }}
								</td>
								<td>
									@if(!empty($model->hasPromotedMember))
										{{ ucfirst($model->hasPromotedMember->total_service)??'N/A' }}
									@else
										{{ $model->total_service??'N/A' }}
									@endif
								</td>
								<td>
									{{ $model->mob_no??'N/A' }}
								...
								<td>
									{{ $model->present_address??'N/A' }}
								</td>
								<td>
									{{ $model->permanent_address??'N/A' }}
								</td>
							</tr>
						@endforeach
					</tbody>
