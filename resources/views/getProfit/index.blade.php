...
<br />
	<div class="panel panel-dark">
		<div class="panel-heading">
		  <a href="{{ route('GetProfit.create') }}" class="btn btn-default c" style="float: right;margin-right: 1%;margin-top: -8px;"><i class="fa fa-plus"></i> Create Return Fund</a>
			  ...
			  <tbody class='ajax_content'>
				...
				@foreach($models as $model)
					...
					<tr>
						...
						<td>
							@if(!empty($model->hasPromotedMember))
								{{ $model->hasPromotedMember->hasMember->name??'N/A' }} | 
						...
						<td>
							<label for="" class="label label-info">{{ $model->hasProfitRate->rate??'--' }}%</label>
						</td>
						<td>
							<label for="" class="label label-primary">{{ number_format((int)$model->paid_amount)??'--' }}</label>
						</td>
						<td>
							<label for="" class="label label-info">{{ number_format((int)$model->profit_amount)??'--' }}</label>
						</td>
						<td>
							<label for="" class="label label-primary">{{ $model->total_amount??'--' }}</label>
						</td>
						<td>
						...
						<td>
							...
							@else
								...
								<a href="{{ url('reciept') }}/{{ $model->id }}" class="btn btn-info btn-sm" title="Payment Released Reciept"><i class="fa fa-eye"></i> Reciept</a>
							@endif
						</td>
					</tr>
				@endforeach
