          ...
          <tbody class='ajax_content'>
            ...
            @foreach($members as $member)
              ...
              <tr>
                <td>{{ $counter }}.</td>
                <td>{{ $member->p_no }}</td>
                <td>{{ ucfirst($member->hasUser->name??'--') }} | {{ $member->hasRank->name??'--' }}</td>
                <td>{{ $member->hasRank->category??'--' }}</td>
                <td>
                  @php 
                ...
                <td>
                    <span class='label label-info'>
                        @if(!empty($member->hasPayment))
                            {{ $amount_converter->gettypenumber($member->hasPayment->sum('sub_monthly_install')) }}
                        @else
                            {{ "0" }}
                        @endif
                    </span>,
