        ...
        <table id="myTable" class="display" cellspacing="0" width="100%">
          ...
          <tbody>
            @foreach($members as $member)
              <tr>
                <td>
                  @if(!empty($member->hasPayment) && $member->hasPayment->payment_status==1)
                    <span style='color:green'><i class='fa fa-check'></i></span>
                  @elseif(empty($member->hasPayment))
                    <span style='color:orange'><i class="fa fa-exclamation-triangle"></i></span>
                  @else
                    <span style='color:#357ebd'><i class='fa fa-spinner'></i></span>
                  @endif
                </td>
                <td>
                  @if(!empty($member->hasPromotedMember))
                    {{ $member->hasPromotedMember->new_p_no??'N/A' }}
                  @else
                    {{ $member->p_no??'N/A' }}
                  @endif
                </td>
                <td>
                  {{ ucfirst($member->name) }} | 
                  @if(!empty($member->hasPromotedMember))
                    {{ $member->hasPromotedMember->hasPromotedRank->name??'N/A' }}
                  @else
                    {{ $member->hasRank->name??'N/A' }}
                  @endif
                </td>
                <td>
                ...
                <td>
                  @php 
                    ...
                    if(!empty($member->hasMemberPaidAmount)){
                      $paid_amount = $member->hasMemberPaidAmount->sum('sub_monthly_install');
