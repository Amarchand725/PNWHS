                ...
                @if($member_details->plotassigned != 1)
                  ...
                  @else
                    @if($member_payment->is_active==0)
                      @if(!empty($member_details->hasPromotedMember))
                        @php $total += $member_details->hasPromotedMember->hasPromotedRank->hasPolicy->hasPaymentPolicy->registration_payment; @endphp
                      @else 
                    ...
                    @else
                      @if($total_installs_amount < 0)
                        @php 
                          $total += $total_installs_amount; 
                          $total_installs_amount = number_format($total_installs_amount);
                        @endphp
                      @endif
                    @endif
                  @endif
                  ...
                  @else
                    @if($member_payment->is_active==0)
                      @if(!empty($member_details->hasPromotedMember))
                        @php $total += $member_details->hasPromotedMember->hasPromotedRank->hasPolicy->hasPaymentPolicy->insurance_payment; @endphp
                      @else 
                        @php $total += $member_details->hasRank->hasPolicy->hasPaymentPolicy->insurance_payment; @endphp
                      @endif
                    @endif
                  ...
                  @if(!empty($member_payment) && $member_payment->is_active==1)
                    <tr>
                      ...
                      <th>
                        @if(!empty($member_details->hasPromotedMember))
                          @php $total += $member_details->hasPromotedMember->hasPromotedRank->hasPolicy->hasPaymentPolicy->monthly_instalment; @endphp
                        @else 
                  ...
                  <tr>
                    ...
                    <td>
                      @if(!empty($member_payment) && $member_payment->is_active != 1)
                        @if($member_payment->submitted_amount<$total)
                          <?php $total_installs_amount = $total-$paid_amount; ?>
                          <span class='label label-info'>{{ number_format($total-$paid_amount) }}</span>
                        @endif
                      @else
                        @if(!empty($member_payment) && date('m', strtotime($member_payment->created_at)) != date('m'))
                  ...
                  <tr>
                    ...
                    <td>
                      @if($total_installs_amount < 0 )
                        <span class='label label-success'>{{ number_format($total_installs_amount) }}</span>
                      @elseif(!empty($member_payment) && $member_payment->is_active != 1)
                        <span class='label label-danger'>{{ number_format($total_installs_amount) }}</span>
                      @else
