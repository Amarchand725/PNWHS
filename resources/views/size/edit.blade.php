        ...
        <div class="col-md-6 col-sm-6">
            ...
            <div class="col-md-6 col-sm-6">
                <div class="form-group">
                    {!! Form::label("propertytype", "Property Type", ["class" => "control-label "]) !!}
                    <span style='color:red;'>*</span>
                    <select name="property_type" id="type" class='form-control select porperty'>
                    <option value=''>Select Type</option>
                    <?php
                        $property_id= DB::table('property_type')->get(); 
                        foreach ($property_id as $key => $type) {
                            ?>
                            <option <?php if($model->property_type == $type->id){echo 'selected';} ?> value='<?= $type->id?>'>{{$type->name}}</option>
                            <?php
                        }
                    ?>
                    </select>
